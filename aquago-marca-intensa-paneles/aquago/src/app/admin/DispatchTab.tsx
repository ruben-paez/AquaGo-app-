...
import { IconCheck, IconClock, IconLock, IconMapPin, IconPhone, IconTruck } from "@/components/icons";
      ...
      <div>
        ...
        {data.queue.length === 0 ? (
          <div className="mt-3 rounded-2xl border border-dashed border-ink/20 bg-white p-8 text-center text-sm text-ink-soft">
            Todos los pedidos tienen repartidor asignado. <IconCheck className="inline h-4 w-4" />
          </div>
        ) : (
                          ...
                          {q.candidates.map((c, i) => (
                            ...
                            >
                              <td className="p-2.5 font-semibold">
                                <IconTruck className="mr-1 inline h-3.5 w-3.5" /> {c.name}
                                {i === 0 && c.eligible && (
          ...
          {data.drivers.map((d) => {
            ...
            return (
              <div key={d.id} className="rounded-2xl border border-ink/10 bg-white p-4 shadow-card">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="grid h-9 w-9 place-items-center rounded-xl bg-water-50 text-water-700"><IconTruck className="h-5 w-5" /></span>
                    <div>
...
function AccessForm({ driverId }: { driverId: number }) {
  ...
  if (!open) {
    return (
      ...
      >
        <IconLock className="mr-1 inline h-4 w-4" /> Crear usuario de acceso para este repartidor
      </button>
    );
  }
