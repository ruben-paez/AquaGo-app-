...
import { IconLock, IconPencil, IconRefresh, IconTrash, IconTruck } from "@/components/icons";
...
function BrandAccessForm({ brand, onSaved }: { brand: BrandAdmin; onSaved: () => void }) {
  ...
  return (
    <div className="rounded-xl border border-water-200 bg-water-50/60 p-3">
      <p className="text-xs font-bold uppercase tracking-wider text-ink-soft">
        <IconLock className="mr-1 inline h-4 w-4" /> Acceso de la marca al panel {brand.accessEmail && <span className="text-teal-700">· actual: {brand.accessEmail}</span>}
      </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-sm font-bold">
            <IconTruck className="mr-1 inline h-4 w-4" /> {vendor.name}
            {!vendor.active && (
        ...
        <div className="flex flex-wrap items-center gap-2">
          {vendor.user ? (
            <span className="rounded-full bg-teal-50 px-2.5 py-1 text-[11px] font-bold text-teal-700 ring-1 ring-teal-200">
              <IconLock className="mr-1 inline h-3 w-3" />{vendor.user.email}
            </span>
          ) : (
          ...
          >
            {vendor.user ? (<><IconLock className="mr-1 inline h-3 w-3" />Resetear acceso</>) : (<><IconLock className="mr-1 inline h-3 w-3" />Crear acceso</>)}
          </button>
          <button
          ...
          >
            <IconPencil className="mr-1 inline h-3 w-3" /> Editar
          </button>
          <button
            onClick={async () => {
              setBusy(true);
              try {
                await fetch(`/api/admin/drivers/${vendor.id}`, {
                  method: "PATCH",
                  headers: sessionHeaders({ "Content-Type": "application/json" }),
          ...
          >
            {vendor.active ? (<><IconTrash className="mr-1 inline h-3 w-3" /> Dar de baja</>) : (<><IconRefresh className="mr-1 inline h-3 w-3" /> Reactivar</>)}
          </button>
        </div>
      </div>
