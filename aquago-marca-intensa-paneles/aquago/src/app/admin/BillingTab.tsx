...
import { IconAlert, IconBan, IconBank, IconCash, IconCheck, IconClock } from "@/components/icons";
            ...
            return (
              ...
              >
                <p className={`text-sm font-semibold ${suspendida ? "text-danger" : "text-warn"}`}>
                  {suspendida ? <IconBan className="inline h-4 w-4" /> : <IconAlert className="inline h-4 w-4" />}{" "}<strong>{b.name}</strong> está{" "}
                ...
                {st && (
                  ...
                  >
                    <IconCheck className="mr-1 inline h-4 w-4" /> Marcar pagada y reactivar
                  </button>
                )}
              </div>
            );
