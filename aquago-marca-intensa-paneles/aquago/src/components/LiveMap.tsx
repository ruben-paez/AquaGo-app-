...
function svgGlyph(paths: string) {
  return `<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
}

const COLORS: Record<MarkerKind, { fill: string; glyph: (n?: number) => string }> = {
  driver: { fill: "#1d4ed8", glyph: () => svgGlyph('<path d="M2.5 6.5h11v10h-11zM13.5 10h4l3 3.2v3.3h-7"/><circle cx="7" cy="17.5" r="1.9"/><circle cx="17" cy="17.5" r="1.9"/>') },
  store: { fill: "#105c88", glyph: () => svgGlyph('<path d="M4.5 9l1-4h13l1 4M5.5 9v11h13V9M4.5 9h15M10 20v-5.5h4V20"/>') },
  stop: { fill: "#0f766e", glyph: (n) => String(n ?? "?") },
  dest: { fill: "#b45309", glyph: () => svgGlyph('<path d="M3.5 10.5 12 3.5l8.5 7"/><path d="M5.5 9.5V20h13V9.5M10 20v-5.5h4V20"/>') },
};
